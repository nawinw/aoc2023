...
int main(int argc, char* argv[])
{
    ...
    int *num_supports = malloc(sizeof(int) * num_bricks);
    memset(num_supports, 0, sizeof(int) * num_bricks);
    ...
    for (int i = 0; i < num_bricks; i++)
    {
        // i can be removed if every brick it supports has more than 1 support
        int valid = 1;
        for (int j = 0; j < num_bricks; j++)
        {
            if (support[i * num_bricks + j] == 1 && num_supports[j] < 2)
            {
                valid = 0;
                break;
            }
